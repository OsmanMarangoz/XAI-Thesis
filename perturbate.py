...
class Perturbation:
    ...
    def __init__(
        ...
        if isinstance(perturbation_function, str):
            ...
            elif perturbation_function == "uniform":
              rng = np.random.default_rng(seed=42)

              self.perturbation_function = lambda x: rng.uniform(
                  low=-128.0, high=128.0,size=x.shape
                )
...
class PerturbationAnalysis:
    ...
    def __init__(
        ...
        if not self.recompute_analysis:
            ...
            for XX, YY in self.generator:
                ...
                '''
                if self.verbose: # Nur wenn verbose=True
                  print("DEBUG: Teste Genauigkeit auf gesammelten X_all, Y_all vor Perturbation", flush=True)
                  temp_X = np.array(X)
                  temp_Y = np.array(Y)

                  # Erstelle ein temporäres, deterministisches Dataset aus den gesammelten NumPy-Arrays
                  # Batch-Größe vom Originalgenerator nehmen, falls vorhanden, sonst Standardwert
                  batch_s = self.generator.batch_size if hasattr(self.generator, 'batch_size') and self.generator.batch_size is not None else 32
                  #check ob generator object leer ist.
                  if len(temp_X) > 0 and len(temp_Y) > 0:
                      try:
                          temp_ds_collected = tf.data.Dataset.from_tensor_slices((temp_X, temp_Y)).batch(batch_s)
                          debug_eval_metrics_collected = self.model.evaluate(temp_ds_collected, verbose=0)
                          print(f"DEBUG: model.evaluate auf gesammelten Daten: Metriken={debug_eval_metrics_collected}", flush=True)
                          if len(debug_eval_metrics_collected) > 1:
                              print(f"DEBUG: Genauigkeit auf gesammelten Daten: {debug_eval_metrics_collected[1]*100:.2f}%", flush=True)
                      except Exception as e:
                          print(f"FEHLER beim Erstellen/Evaluieren des temporären Datasets aus gesammelten Daten: {e}", flush=True)
                          print(f"Shapes: temp_X: {temp_X.shape if hasattr(temp_X, 'shape') else 'N/A'}, temp_Y: {temp_Y.shape if hasattr(temp_Y, 'shape') else 'N/A'}", flush=True)
                  else:
                      print("DEBUG: X_all oder Y_all sind leer, Test übersprungen.", flush=True)
                  '''
            X = np.array(X)
            Y = np.array(Y)
        ...
        self.verbose = verbose
        print("DEBUG: alles korrekt abgelaufen.", flush=True)
    ...
    def _predict_and_calc_accuracy(self, data_source, is_already_batched_numpy=False, numpy_batch_size=32):
        """
        Nimmt einen Generator oder (X_np, Y_np) entgegen, führt Batch-weise model.predict aus,
        sammelt alle Ergebnisse und berechnet die Gesamtgenauigkeit.
        """
        collected_predictions = []
        collected_labels = []

        iterator_source = data_source
        if not is_already_batched_numpy: # Es ist ein Tupel (all_X_np, all_Y_np)
            all_X_np, all_Y_np = data_source
            # Erzeuge einen temporären Generator für Batch-weise Verarbeitung der NumPy-Arrays
            # tf.data.Dataset funktioniert auch mit tf.compat.v1.disable_eager_execution() für Input
            temp_dataset = tf.data.Dataset.from_tensor_slices((all_X_np, all_Y_np)).batch(numpy_batch_size)
            iterator_source = temp_dataset # Jetzt ist es ein Iterator

        for images_batch, labels_batch in iterator_source:
            # images_batch ist entweder direkt NumPy (von BatchSequence) oder Tensor (von tf.data.Dataset)
            # model.predict kann beides verarbeiten.
            batch_predictions = self.model.predict(images_batch, verbose=0, 
                                                   batch_size=images_batch.shape[0] if images_batch.shape[0] is not None else None)
            collected_predictions.append(batch_predictions)

            # Labels zu NumPy konvertieren, falls sie Tensoren sind (von tf.data.Dataset)
            if hasattr(labels_batch, 'numpy'):
                labels_batch_np = labels_batch.numpy()
            else:
                labels_batch_np = labels_batch # Ist bereits NumPy (von BatchSequence)
            collected_labels.append(labels_batch_np)

        if not collected_predictions:
            if self.verbose: print("WARNUNG: _predict_and_calc_accuracy - Keine Vorhersagen gesammelt.", flush=True)
            return [0.0, 0.0] # [dummy_loss, accuracy]

        final_predictions_np = np.concatenate(collected_predictions, axis=0)
        final_labels_np = np.concatenate(collected_labels, axis=0)

        pred_classes = np.argmax(final_predictions_np, axis=1)
        true_classes = np.argmax(final_labels_np, axis=1) 

        accuracy = np.mean(pred_classes == true_classes)
        
        if self.verbose:
             print(f"DEBUG: _predict_and_calc_accuracy - Genauigkeit: {accuracy*100:.2f}% auf {final_predictions_np.shape[0]} Samples.", flush=True)
        return [0.0, accuracy] # Dummy-Verlust, Genauigkeit
